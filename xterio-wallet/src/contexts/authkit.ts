...
export const useConnect = () => {
  ...
  const connect = useCallback(async ({ chain, jwt = '' }: { chain?: ChainInfo; jwt?: string }) => {
...
export const useCustomize = () => {
  const [erc4337, _setERC4337] = useState<ERC4337Options | undefined>(undefined)
...
export const getEVMPublicAddress = async ({ chainId, erc4337 }: { chainId?: number; erc4337?: ERC4337Options }) => {
